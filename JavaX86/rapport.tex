...
Ces registres peuvent intéragir entre eux, exemples, les expression arithmétiques ou logiques entre deux registres.
...
Ce sera cette classe qui sera utilisée dans la partie interpretation graphique pour ne pas avoir à rajouter la gestion de la création.


\subsection{Register}
La classe Register est la classe principale du programme.
En effet c'est dans cette classe que l'on vas définir l'ensemble des variables pour la création et la gestion des registres.
...
4 - Boucle qui vas itérer sur l'entiereté de l'ArrayOfBit du registre 1

5 - Récupération du bit avec comme index i (utile car modifié par la suite)

6 - Cette ligne calcul la valeur du bit i à l'aide de 2 xor effectué sur la retenue, r1 et r2.
Cette ligne représente l'addition binaire de ces 2 bits.

7 - Pour finir, cette ligne calcul la valeur de la retenue pour le prochain bit.
La retenue est vraie si au moins 2 des 3 valeurs (la retenue actuelle et les 2 registres) sont vrais.
...
Il existe aussi des fonctions sur les piles :
\begin{itemize}
